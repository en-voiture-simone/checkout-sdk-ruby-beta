  ...
  describe '.increment_payment_authorization' do
    context 'when making a payment authorization' do
      it 'increments payment correctly' do
        ...
        response = default_sdk.payments_client.increment_payment_authorization(payment_response.id, request)
      ...
      it 'make increment payment idempotent' do
        ...
        increment_1 = default_sdk.payments_client.increment_payment_authorization(payment_response.id,
                                                                                  request,
                                                                                  idempotency_key)

        assert_response increment_1

        increment_2 = default_sdk.payments_client.increment_payment_authorization(payment_response.id,
                                                                                  request,
                                                                                  idempotency_key)
...
def make_authorization_payment
  ...
  response = default_sdk.payments_client.request_payment(request)
