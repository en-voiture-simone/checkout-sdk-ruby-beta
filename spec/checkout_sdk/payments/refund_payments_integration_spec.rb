  ...
  describe '.refund_payment' do
    context 'when refunding a payment' do
      it 'should refund correctly with balances' do
        ...
        proc = lambda { default_sdk.payments_client.refund_payment(payment_response.id, request) }
        ...
        payment_details = default_sdk.payments_client.get_payment_details(payment_response.id)
      ...
      it 'should refund idempotent correctly' do
        ...
        proc = lambda { default_sdk.payments_client.refund_payment(payment_response.id, request, idempotency_key) }
        proc2 = lambda { default_sdk.payments_client.refund_payment(payment_response.id, request, idempotency_key) }
